...
### 兼容旧版 `/exec check_feeds()`

如果你仍在使用旧脚本中的 `/exec check_feeds()`，插件会调用监控逻辑读取说说、执行自动评论/回复，并在完成后返回 `success` 或具体的失败原因。该命令同样会更新 `processed_list.json`，因此不会破坏已有的去重机制。
...
如果只想测试 LLM 是否能生成合适的说说文案，而不实际发送，可使用新增的 `/exec qzone_live_post_llm()`：

```text
/exec qzone_live_post_llm()
```

命令会调用 Nerko Agent 配置的文本模型生成一段 80 字以内的说说正文，并返回示例内容。也可以通过 `/exec qzone_live_post_llm("请写一段有关夏日海边的分享")` 自定义提示词，观察不同 prompt 的输出效果。
