...
from .qzone_api import create_qzone_api, ensure_qzone_api
...
async def send_feed(message: str,
    ...
    if qzone is None:
        logger.error("send_feed: 无法创建 QzoneAPI，取消发送")
        return False
...
async def read_feed(target_qq: str, num: int) -> list[dict]:
    ...
    qzone = await ensure_qzone_api()
    if qzone is None:
        logger.error("read_feed: 无法创建 QzoneAPI，取消读取")
        return [{"error": "无法创建QzoneAPI，请检查cookie"}]
...
async def monitor_read_feed(self_readnum) -> list[dict]:
    ...
    qzone = await ensure_qzone_api()
    if qzone is None:
        logger.error("monitor_read_feed: 无法创建 QzoneAPI，取消监控读取")
        return []
...
async def like_feed(target_qq: str, fid: str) -> bool:
    ...
    qzone = await ensure_qzone_api()
    if qzone is None:
        logger.error("like_feed: 无法创建 QzoneAPI，取消点赞")
        return False
...
async def comment_feed(target_qq: str, fid: str, content: str) -> bool:
    ...
    qzone = await ensure_qzone_api()
    if qzone is None:
        logger.error("comment_feed: 无法创建 QzoneAPI，取消评论")
        return False
...
async def reply_feed(fid: str, target_qq: str, target_nickname: str, content: str, comment_tid: str) -> bool:
    ...
    qzone = await ensure_qzone_api()
    if qzone is None:
        logger.error("reply_feed: 无法创建 QzoneAPI，取消回复")
        return False
